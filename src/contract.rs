use cosmwasm_std::{
    coins, entry_point, to_json_binary, Binary, Deps, DepsMut, Env, MessageInfo, Reply, Response,
    StdResult, Uint128,
};
...
mod exec {
    ...
    use super::*;
    use crate::{
        msg::{DepositMsg, PositionType, WithdrawMsg},
        state::{price_function_inv, raw, Weight},
    };
    ...
    pub fn withdraw(
        ...
        if withdrawal_address == env.contract.address {
            return Err(ContractError::CantWithdrawToContract(
                withdrawal_address.into(),
            ));
        }
        ...
        let shares_burn_response = execute_burn(deps, env.clone(), info, shares).map_err(|_| {
            ContractError::InalidWithdrawalAmount {
